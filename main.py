...
async def stop_generation(gen_id: str):
    ...
    gen = generations[gen_id]
        ...
        # Cannot stop a generation that's actively processing (MLX inference can't be interrupted)
...
async def get_audio_track(gen_id: str, track_idx: int, format: Optional[str] = None):
    ...
    if format:
        ...
        if fmt not in ("wav", "flac", "mp3"):
            raise HTTPException(400, "Invalid format. Use wav, flac, or mp3.")
        candidate = output_path.with_suffix(f".{fmt}")
        if candidate.exists():
            output_path = candidate
        else:
            raise HTTPException(404, f"Requested format '{fmt}' not found")
    else:
